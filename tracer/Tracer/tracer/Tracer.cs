...
namespace Tracer.tracer
{
    ...
    {
        private ConcurrentDictionary<int, ThreadInfo> _workingThreads;  //это thread-safe потокобезопасная коллекция
                 //защищает от обращения к одной переменной                                           
        ...
        public void StartTrace()
        {
            ...
            var stack = _workingThreads.GetOrAdd(threadId, new ThreadInfo()); //добавляет если не существует
                                                                                        //возвращает если существует
            var stackTrace = new StackTrace(true);
            var stackFrames = stackTrace.GetFrame(1);   //0 это startTrace  stackframe - кадр стека
            var methodName = stackFrames.GetMethod().Name; //метод который работает в тестах
